...
export default class ModalDropdown extends Component {
  ...
  _updatePosition(callback) {
    if (this._button && this._button.measure) {
      ...
      UIManager.measure(ReactNative.findNodeHandle(this._button), (x, y, width, height, pageX, pageY) => {
        this._buttonFrame = {x: pageX, y: pageY, w: width, h: height};
        callback && callback();
      });
    }
  }
  ...
  _renderButton() {
    ...
    return (
      ...
      <TouchableNativeFeedback  ref={button => this._button = button}
                                disabled={disabled}
                                background={TouchableNativeFeedback.Ripple('#a2a4a6',false)}
  ...
  _renderDropdown() {
    ...
    return (
      <ListView scrollEnabled={scrollEnabled}
                pageSize={parseInt(this.props.options.length/2)}
                style={styles.list}
